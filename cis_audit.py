...
class CISAudit:
    ...
    def audit_kernel_module_is_disabled(self, l_mname: str, l_mtype: str) -> int:
        ...
        def module_check() -> str: 
            ...
            l_searchloc = ["/lib/modprobe.d/*.conf", "/usr/local/lib/modprobe.d/*.conf",
                           "/run/modprobe.d/*.conf", "/etc/modprobe.d/*.conf"]
            l_mpath = "/lib/modules/**/kernel/" + l_mtype
            l_mpname = l_mname.replace('-', '_')
            l_mndir = l_mname.replace('-', '/')

            def module_loadable_chk():
                nonlocal l_output, l_output2, state
                ...
                try:
                    ...
                    if l_loadable.count('\n') > 1:
                        l_loadable_lines = l_loadable.splitlines()
                        l_loadable = '\n'.join([line for line in l_loadable_lines
                                               if re.search(r"(^\s*install|\b" + l_mname + r")\b", line)])

                    if re.search(r'^\s*install\s+/bin/(true|false)', l_loadable):
                        l_output += f"\n - module: \"{l_mname}\" is not loadable: \"{l_loadable}\""
                    else:
                        l_output2 += f"\n - module: \"{l_mname}\" is loadable: \"{l_loadable}\""
                        state = 2
                except subprocess.CalledProcessError:
                    l_output += f"\n - Error checking if module: \"{l_mname}\" is loadable"

            def module_loaded_chk():
                nonlocal l_output, l_output2, state
                # Check if the module is currently loaded
                try:
                    lsmod_output = subprocess.check_output(["lsmod"], universal_newlines=True)
                    if l_mname not in lsmod_output:
                        l_output += f"\n - module: \"{l_mname}\" is not loaded"
                    else:
                        l_output2 += f"\n - module: \"{l_mname}\" is loaded"
                except subprocess.CalledProcessError:
                    l_output += f"\n - Error checking if module: \"{l_mname}\" is loaded"

            def module_deny_chk():
                nonlocal l_output, l_output2, l_dl, state
                # Check if the module is deny listed
                l_dl = "y"
                try:
                    showconfig_output = subprocess.check_output(["modprobe", "--showconfig"],
                                                              universal_newlines=True)
                    if re.search(r'^\s*blacklist\s+' + l_mpname + r'\b', showconfig_output, re.MULTILINE):
                        # Find which file contains the blacklist
                        blacklist_files = []
                        for pattern in l_searchloc:
                            for filename in glob.glob(pattern):
                                with open(filename, 'r') as f:
                                    if re.search(r'^\s*blacklist\s+' + l_mname + r'\b', f.read(), re.MULTILINE):
                                        blacklist_files.append(filename)

                        l_output += f"\n - module: \"{l_mname}\" is deny listed in: \"{' '.join(blacklist_files)}\""
                    else:
                        l_output2 += f"\n - module: \"{l_mname}\" is not deny listed"
                        state = 2
                except subprocess.CalledProcessError:
                    l_output += f"\n - Error checking if module: \"{l_mname}\" is deny listed"

            # Check if the module exists on the system
            for l_mdir_pattern in glob.glob(l_mpath):
                l_module_dir = os.path.join(l_mdir_pattern, l_mndir)
                if os.path.isdir(l_module_dir) and os.listdir(l_module_dir):
                    l_output3 += f"\n - \"{l_mdir_pattern}\""
                    if l_dl != "y":
                        module_deny_chk()
                    ...
                    curr_kernel_path = f"/lib/modules/{kernel_version}/kernel/{l_mtype}"
                    if l_mdir_pattern == curr_kernel_path:
                        module_loadable_chk()
                        module_loaded_chk()
                else:
                    l_output += f"\n - module: \"{l_mname}\" doesn't exist in \"{l_mdir_pattern}\""
...
Examples:
    ...
    parser.add_argument('--fix', action='store_const', const='True', dest='fix', help='Run script and execute remediation on ran tests')
